...
void handleImageButtonEvents(GImageButton iButton)
{
  //The about window
  ...
  if(iButton == createSlides && iButton.eventType == GButton.CLICKED)
  {
    ...
    tableTopic = new GLabel(this, "DEFAULT", int(screen.width * 0.05), int(screen.height * 0.1), int(screen.width * .8), int(screen.height * 0.5));
    tableTopic.setFont("Arial", int(50 * screen.width/1440));
    ...
    slideNext = new GImageButton(this, null, foiles, int(screen.width * .9) - 100, int(screen.height * .8));
    slideShow.add(slideNext);
    
    foiles = new String[]{"BackButtonDefault.png", "BackButtonMOver.png", "BackButtonClicked.png"};
    slideBack = new GImageButton(this, null, foiles, int(screen.width * .1), int(screen.height * .8));
...
String generator(ArrayList inputFiles, int seeder)
{
  ...
  String pickedSentence = (String) usedSentences.get(int(random(0, usedSentences.size())));
  ...
  for(int i = 0; i < 3; i++)
  {
    for(int j = 0; j < oldSentences.size(); j++)
    {
      ...
      if(pickedSentence.equals(testSentence) == true)
      {
        ...
        pickedSentence = (String) usedSentences.get(int(random(0, usedSentences.size())));
      }
    }
  }
  ...
  for(int i = 0; i < libCounter.size(); i++)
  {
    ...
    randLib = int(random(0, usedLibs.size()));
...
class Category
{
  ...
  boolean testCode(String inputCode)
  {
  ...
  ArrayList returnLibs (String inputCode)
  {
    ArrayList collectLibs = (ArrayList) hmLibs.get(inputCode);
    return collectLibs;
  }
  
  ArrayList returnSentences()
  {
    return sentences;
  }
}
